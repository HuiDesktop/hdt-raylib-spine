...
void drawSkeleton(spSkeleton* skeleton, bool PMA) {
    ...
    for (int i = 0; i < skeleton->slotsCount; ++i) {
        ...
        float tintR = skeleton->r * slot->r * alpha;
        float tintG = skeleton->g * slot->g * alpha;
        float tintB = skeleton->b * slot->b * alpha;
        ...
        if (attachment->type == SP_ATTACHMENT_REGION) {
            ...
            if (slot->data->additiveBlending != blend_mode)
            {
                EndBlendMode();
                blend_mode = slot->data->additiveBlending;
                switch (blend_mode)
                {
                default: //Normal
            ...
            // to fit the mesh.
            if (mesh->verticesCount > MAX_VERTICES_PER_ATTACHMENT / 2) continue;
            ...
            // for simplicity, in production you want to reallocate the scratch buffer
            // to fit the mesh.
            if (mesh->uvsCount > MAX_VERTICES_PER_ATTACHMENT / 2) continue;
            ...
            // bone to which the slot (and hence attachment) is attached has been calculated
            // before rendering via spSkeleton_updateWorldTransform
            spSkinnedMeshAttachment_computeWorldVertices(mesh, slot, worldVerticesPositions);
